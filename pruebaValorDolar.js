...
(async function main() {
 ...
 // console.log("ventana abierta");
  ...
  fs.writeFileSync('screenshot4.png', screenshot4, 'base64');
  console.log("screenshot4 tomado");

  try {
    await driver.findElement(By.xpath(imput)).sendKeys("500");
  } catch (error) {
    console.error(`error en el imput: ${error}`);
    await driver.quit();
    throw Error("error en el imput");
  }
  console.log("imput ingresado");

  const screenshot5 = await driver.takeScreenshot();
  fs.writeFileSync('screenshot5.png', screenshot5, 'base64');
  console.log("screenshot5 tomado");

  try {
    const imput = await driver.findElement(By.xpath(blue));
    const valorIngresado = await imput.getAttribute("value"); // Obtiene el valor del campo de entrada
    console.log(`valor ingresado: ${valorIngresado}`);
  } catch (error) {
    console.error(`error en valor blue: ${error}`);
    await driver.quit();
    throw Error("error en valor blue");
  }
  console.log("valor blue correcto");

  const screenshot6 = await driver.takeScreenshot();
  fs.writeFileSync('screenshot6.png', screenshot6, 'base64');
  console.log("screenshot6 tomado");
